...
def parse_request_llm(state: ProcurementState) -> dict:
    ...
    prompt = f"""
Extract quantity from this request.
Return ONLY JSON: {{"quantity": int}}
...
def lookup_vendors(state: ProcurementState) -> dict:
    ...
    vendors = [
        {"name": "Dell", "id": "V-001", "category": "laptops", "rating": 4.5, "delivery_days": 5},
        {"name": "Lenovo", "id": "V-002", "category": "laptops", "rating": 4.3, "delivery_days": 7},
        {"name": "HP", "id": "V-003", "category": "laptops", "rating": 4.1, "delivery_days": 4},
    ]
    for v in vendors:
...
def fetch_pricing(state: ProcurementState) -> dict:
    ...
    print("\n[Step 2] Fetching prices + building quotes...")
    ...
    for v in vendors:
        ...
        response = llm_with_tools.invoke(
            f"Get unit price for {vendor} using get_unit_price tool."
        )

        tool_call = response.tool_calls[0]
        unit_price = get_unit_price(**tool_call["args"])
        ...
        print(f"   {vendor}: €{unit_price} x {qty} = €{total:,}")
...
builder.add_node("parse_request", parse_request_llm)
